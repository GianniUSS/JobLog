...
function setOfflineMode(value, options) {
    ...
    if (!offlineMode) {
        ...
        showPopup('?? Modalità offline attiva. Mostro i dati salvati');
...
async function toggleSelectionPause() {
    ...
    if (eligibleKeys.length === 0) {
        showPopup(
            intent === "pause"
                ? "?? Nessun operatore in attività nella selezione"
...
async function finishSelection() {
    ...
    await performSelectionAction(
        ...
        "/api/member/finish",
        "? Attività concluse",
        "?? Impossibile chiudere le attività selezionate"
    );
}
...
function handleTeamAddActivityClick(event) {
    ...
    if (!projectVisible) {
        showPopup("?? Carica un progetto per aggiungere attività");
        return;
    }
...
async function startActivity(activityId) {
    ...
    try {
        ...
        showPopup("?? Impossibile avviare l'attività");
    }
}
    ...
    try {
        ...
        if (errors.length > 0) {
            ...
            if (errorMsg.includes('missing_external_id') || errorMsg.includes('ID esterno')) {
                ...
                showPopup("?? Nessuna attività assegnata");
            } else if (errorMsg.includes('already_running')) {
                showPopup("?? Timer già in esecuzione");
            } else {
...
async function submitNewActivity(event) {
    ...
    if (!payload.label) {
        showPopup("?? Inserisci il nome dell'attività");
        return;
    }
    const saveBtn = document.getElementById("newActivitySaveBtn");
    ...
    try {
        ...
        try {
            ...
            showPopup("?? Attività creata, ricarica se non la vedi");
        }
        ...
        if (status === 409) {
            showPopup("?? Codice attività già in uso o progetto non disponibile");
        } else if (status === 400) {
            showPopup("?? Dati attività non validi");
        } else {
            showPopup("?? Impossibile creare l'attività");
        }
    } finally {
...
async function finishAll() {
    try {
        ...
        showPopup("?? Impossibile chiudere le attività");
    }
}
    ...
    try {
        if (memberKeys.length > 0) {
            ...
            success = await performSelectionAction(
                ...
                "/api/member/finish",
                "",
                "?? Impossibile chiudere l'attività"
            );
        }
        if (success) {
...
function requestMoveSelection() {
    ...
    if (cachedActivities.length === 0) {
        showPopup("?? Nessuna attività disponibile");
        return;
    }
    openActivityModal();
}
    ...
    try {
        ...
        if (res.ok) {
            ...
            const warning =
                ...
                "?? Impossibile ricaricare: attività in corso";
            showPopup(warning);
            await refreshState();
        } else {
    ...
    try {
        ...
        if (!res.ok || !data.ok) {
            ...
            if (errorMsg === "already_in_project") {
                showPopup("?? Operatore già nel progetto");
            } else {
                showPopup("?? " + errorMsg);
            }
            return;
